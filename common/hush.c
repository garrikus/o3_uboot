...
static int run_pipe_eal(struct pipe *pi)
{
	int i;
	int nextin;
	int flag = do_repeat ? CMD_FLAG_REPEAT : 0;
...
static int run_eal(struct pipe *pi)
{
	char *save_name = NULL;
	char **list = NULL;
	char **save_list = NULL;
...
static int run_st(struct pipe *pi)
{
	int rcode=0;
...
int check_kernel_crc(const char *s, int flag)
{
	struct in_str input;
	char *p = NULL;
	int rcode;
