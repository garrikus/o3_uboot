...
$(VERSION_FILE):
		@( printf '#define U_BOOT_VERSION "U-Boot %s%s"\n#define UBT_VERSION_ID "UBT-version-id: %s%s"\n' \
        "$(U_BOOT_VERSION)" '$(shell $(TOPDIR)/tools/setlocalversion $(TOPDIR))' \
        '$(shell $(TOPDIR)/tools/setlocalversion $(TOPDIR))' $(UBT_BTD_ID)) > $@.tmp
...
omap3_orion_config :	unconfig
	@$(MKCONFIG) $(@:_config=) arm arm_cortexa8 orion navis omap3
